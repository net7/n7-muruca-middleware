...
exports.CommonHelper = {
    ...
    getSnippetAroundTag(node_attr, snippet, text) {
        const tag_regex = '<[^>]+?' + this.escapeRegExp(node_attr) + '=["\'\\s]' + this.escapeRegExp(snippet) + '["\'\\s][^>]*?>[^<>]+?<\/[^>]+?>';
        ...
        if (matches) {
            matches.forEach(element => {
                const regex = new RegExp('(<[^>]+?' + this.escapeRegExp(node_attr) + '=["\'\\s]' + this.escapeRegExp(snippet) + '["\'\\s].*?>)(.+?)(<\/[^>]+?>)', 'g');
