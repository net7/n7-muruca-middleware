...
export const spanNear = (queryField: {
  ...
  const in_order =
    typeof queryField.in_order !== 'undefined' ? queryField.in_order : true;
  const x = {
    span_near: {
      clauses: [],
  ...
  if (inner_hits) {
    ...
    if (inner_hits.highlight) {
      ih['highlight'] = {
        ...
        order: 'none',
        number_of_fragments:
          inner_hits.number_of_fragments !== undefined
...
export const buildSortParam = (sort: string, sort_conf) => {
  if (sort === '_score' || sort === 'sort_ASC') {
    ...
    const order =
      sort.slice(lastIndex + 1) != '' ? sort.slice(lastIndex + 1) : 'ASC';
