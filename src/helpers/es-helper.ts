...
export const ESHelper = {
  ...
  buildAggs(facets_request, query_facets) {
    ...
    for (const f in facets_request) {
      ...
      const limit: number =
        facets_request[f].limit != undefined ? facets_request[f].limit : 100;
      const offset: number =
        facets_request[f].offset != undefined ? facets_request[f].offset : 0;
      const size: number = offset + limit;
      const filterTerm: string =
        facets_request[f].query != undefined && facets_request[f].query != ''
          ? facets_request[f].query + '*'
          : '';
      const minDocCount: number =
        query_facets[key].showEmpty != undefined && query_facets[key].showEmpty
          ? 0
          : 1;
      const sortMode: string = query_facets[key].sort;
      const sort: string = sortMode != undefined ? '_key' : '_count';
      ...
      if (nested) {
        if (query_facets[key]['nestedFields']) {
          const build_aggs = this.buildNested(
            query_facets[key]['nestedFields'],
            query_facets[key].search,
            query_facets[key].title,
            size,
            filterTerm,
            query_facets[key]['innerFilterField'],
            extra,
            minDocCount,
            sort,
