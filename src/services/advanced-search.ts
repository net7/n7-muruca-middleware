...
export class AdvancedSearchService {
  ...
  buildAdvancedQuery = (query_params) => {
        ...
        if (query_key) {
          switch (
            ...
            case 'fulltext':
              ...
              let ft_query = this.buildFulltextQuery(
                query_key,
            ...
            case 'term_field_value':
              ...
              const query_field_value = ASHelper.buildQueryString(
                query_params[query_key.query_params.value],
                {
                  allowWildCard: query_key.addStar,
                  stripDoubleQuotes:
                    query_key.stripDoubleQuotes != undefined
                      ? query_key.stripDoubleQuotes
                      : true,
                },
              );
    ...
    if (advanced_conf['search_full_text']) {
      const text_query = this.buildXmlTextQuery(
        advanced_conf.search_full_text,
        query_params,
      );
      if (text_query) {
  ...
  buildXmlTextQuery(advanced_conf, data) {
    ...
    if (xml_query_should.length > 0) {
      const xml_query_nested = ASHelper.nestedQuery(
        advanced_conf.options.path,
        ASHelper.queryBool([], xml_query_should).query,
        inner_hits,
      );
  ...
  parseQueryGroups(search_groups, data, inner_hits) {
    ...
    Object.keys(search_groups).forEach((groupId) => {
        ...
        if (q.length > 0) {
          ...
          if (query_conf.options?.nested) {
            if (query_conf.highlight) {
              nested_innerhits['highlight'] = ASHelper.buildHighlights(
                query_conf.highlight,
              );
            }
            nested_innerhits['name'] = groupId;
            const nested = {
              nested: ASHelper.nestedQuery(
                query_conf.options.nested,
                query_bool,
                nested_innerhits,
              ),
            };
  ...
  buildTextQuery(data, query_conf, groupId, inner_hits) {
    ...
    if (query_conf.options?.nested) {
      if (query_conf.highlight) {
        inner_hits['highlight'] = ASHelper.buildHighlights(
          query_conf.highlight,
        );
      }
