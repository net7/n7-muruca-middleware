...
export class TeipublisherService {
  ...
  getXmlDocument = async (xml: string) => {
    const api_url =
      this.teiPublisherUri + 'document/' + encodeURIComponent(xml);
    const res = await HttpHelper.doRequestNoJson(api_url);
    return res;
  };
  ...
  getNodePath = async (doc: string, path: string) => {
    ...
    const api_url =
      this.teiPublisherUri + 'doc_root_id/' + encodeURIComponent(doc);
