...
export class AdvancedSearchParser implements Parser {
  ...
  async advancedParseResultsItems({ data, options }) {
    ...
    await Promise.all(
      data.map(
        async ({ _source: source, highlight, inner_hits, matched_queries }) => {
          let itemResult = {
            highlights: [],
          };
          if (highlight) {
            for (let prop in highlight) {
              ...
              if (prop != 'text_matches') {
                if (
                  conf[searchId]['noHighlightLabels'] &&
          ...
          if (inner_hits && Object.keys(inner_hits)) {
            for (var prop in inner_hits) {
              ...
              const xml_filename =
                conf[searchId]?.xml_search_options?.field_filename ||
                'xml_filename';
              const doc = xml_filename
                .split('.')
                .reduce((a, b) => a[b], source);
              if (doc) {
                const hh = await this.parseXmlTextHighlight(
                  inn_hits,
                  teiPublisherUri,
                  doc,
                );
  ...
  async parseXmlTextHighlight(inn_hits: any, teiPublisherUri = '', doc = '') {
    ...
    if (xpaths.length > 0 && doc != '') {
      xpath_root_id = await this.getTeipublisherNodesRoot(
        teiPublisherUri,
        doc,
        [...new Set(xpaths)],
      );
    ...
    for (let el in objects) {
      if (objects[el]) {
        ...
        if (xpath_root_id != null) {
          const root = xpath_root_id.find(
            (x) => x.xpath === objects[el].xpath,
          ).root_id;
          hl['link'] = {
  ...
  getXmlLastDivPath(path) {
    ...
    for (let i = path.length - 1; i >= 0; i--) {
      if ((path[i] && path[i].node == 'div') || divFound) {
        divFound = true;
        const node =
          typeof path[i].position !== 'undefined'
    ...
    if (Array.isArray(nodes)) {
      ...
      hit.highlight[prop].forEach((snippet) => {
        if (!attr_parsed.includes(snippet)) {
          ...
          const snippets = CommonHelper.getSnippetAroundTag(
            node_attr,
            snippet,
            decoded_text,
          );
          ...
          if (snippets) {
            snippets.forEach((element) => {
              uniqueSnippets.push(
                "<span class='mrc__text-attr_value'>" +
    ...
    function recursiveSearch(currentData, remainingKeys: string[]): string[] | null {
      ...
      if (key === "_refs" && Array.isArray(currentData[key])) {
        for (const item of currentData[key]) {
          const matchesProperty = Object.values(item).some((value) =>
            value != null && value.toString() === targetValue
          );
          if (matchesProperty) {
