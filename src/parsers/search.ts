    ...
    facets.forEach(({ id, query, offset }) => {
      ...
      if (data[id]) {
        ...
        if (buckets_data && buckets_data.buckets) {
          ...
          buckets_data.buckets.forEach(
            (
              agg: { key: string; doc_count: number; from?: any; to?: any },
              index: number,
            ) => {
              const haystack_formatted = (
                agg.key.split('|||')[0] || ''
              ).toLowerCase();
              const haystack_notFormatted = (
                agg.key.split('|||')[1] || ''
              ).toLowerCase();
              const needle = (query || '').toLowerCase();
              if (
                haystack_formatted.includes(needle) ||
                haystack_notFormatted.includes(needle)
              ) {
                const facet = {
                  ...
                  for (const key in query_facets[id]['extra']) {
                    if (agg[key] && agg[key]['buckets']) {
                      if (agg[key]['buckets'].length == 1) {
                        ...
                        extra_args[key] = agg[key]['buckets'].map((bucket) => {
                          return bucket['key'];
                        });
                      } else {
                        extra_args[key] = null;
          ...
          if (query_facets[id]['sortValues']) {
            values.sort((a, b) => {
              return (
                query_facets[id]['sortValues'].indexOf(a['payload']) -
                query_facets[id]['sortValues'].indexOf(b['payload'])
              );
            });
          }
        }
  ...
  if (keys.includes('buckets')) {
    ...
    keys.forEach((k) => {
      if (k != 'distinctTerms' && typeof data[k] === 'object') {
        ...
        const dt = data['distinctTerms']
          ? data['distinctTerms']['value']
          : null;
