...
describe('home rest controller', function murucaHomeCtrlTest() {
  beforeEach(() => {
    nock('http://net7mock.com').get('/layout/home').reply(200, response);
  });

  context('input missing - ', function () {
    it('should return a 404', async function () {
      let result = await controller.getHomeLayout(null, null, null);
      // Assert
  ...
  context('response correct', function () {
    it('should return correct data', async function () {
      let result = await controller.getHomeLayout(
        {
          body: JSON.stringify([
            'slider-main',
            'content-main',
      ...
      // Assert
      expect(data)
        .to.have.property('slider-main')
        .to.have.property('slides')
        .with.lengthOf(0);
