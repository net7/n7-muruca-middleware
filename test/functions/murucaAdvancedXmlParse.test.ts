...
describe('XML search parse results', function murucaHomeCtrlTest() {
  ...
  context('Parse all document Highlights', function () {
    it('should return highlights for each document', async function () {
      ...
      expect(result).to.have.property("highlights_obj").to.have.property("text.body_0.div_8");
      expect(result).to.have.property("highlights_obj").to.have.property("text.body_0.div_18");
  ...
  context('Format snippet', function () {
    it('should return highlight snippets', async function () {
      ...
      const element = {
        texts: ["Mauris sollicitudin fermentum libero.", "Phasellus magna.", "In turpis."],  
      ...
      expect(text).eq(element.breadcrumb + element.texts[0] + text_divider + element.texts[1] + text_divider + element.texts[2]);
  ...
  context('Format references', function () {
    it('should return formatted references', async function () {
      ...
      let snippet = "Livio, Ab urbe cond.; ps. Aurelio Vittore (Plinio per Petrarca), De viris illustribus; Eutropio";
